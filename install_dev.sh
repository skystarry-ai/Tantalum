...
if ! command -v zig &>/dev/null; then
    ...
    ZIG_VERSION="0.15.2"
    ZIG_TAR="zig-linux-${ARCH}-${ZIG_VERSION}.tar.xz"
    ZIG_URL="https://ziglang.org/download/${ZIG_VERSION}/${ZIG_TAR}"

    curl -fsSL "$ZIG_URL" -o /tmp/$ZIG_TAR
    tar -xf /tmp/$ZIG_TAR -C /tmp
    sudo mv /tmp/zig-linux-${ARCH}-${ZIG_VERSION} /usr/local/zig
    sudo ln -sf /usr/local/zig/zig /usr/local/bin/zig
    rm /tmp/$ZIG_TAR
    success "Zig ${ZIG_VERSION} installed"
else
...
# Zig Gateway Build
# ---------------------------------------------------------------------------
info "Building Tantalum Gateway..."
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "$SCRIPT_DIR/backend/build.zig" ]]; then
    cd "$SCRIPT_DIR/backend"
    zig build -Doptimize=ReleaseSafe
    cd "$SCRIPT_DIR"
    success "Gateway built: backend/zig-out/bin/tantalum-gateway"
else
    dim "backend/build.zig not found, skipping gateway build"
fi
